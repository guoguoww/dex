    ...
    <div class="">
            ...
            <swiper :options="swiperOption" ref="mySwiper" @slideChange='slideChange()'>
                ...
                <swiper-slide v-for="(i,index) in list" :key="index">
                    ...
                    <div class="swiper-right">
                        ...
                        <p class="swiper-right-coin-p">{{i.marginRate}} {{i.unitsCode}}/张</p>
                    </div>
        ...
        <div class="charts">
            <div class="charts-head">
                <dl>
                    <dd>
                        <span>当前价：{{Number(market.price).toFixed(+market.precision)}}</span>
            ...
            <div class="charts-can">
                ...
                <!-- <div id="mchart"></div> -->
                <trading-view v-if="list.length" :code='list[0].marketCode' :market='market' height='21rem' :interval='intervalact'></trading-view> 
            </div>
        ...
        <div class="charts-but" style="pointer-events: auto;">
            <button class="rise" @click="orderpopup(0)">市价买多
                <i class="iconfont icon-jiantou-copy-copy"></i>
            </button>
            <button class="fall" @click="orderpopup(1)">市价卖空
        ...
        <transition name="popup">
            <div id="Business" v-show="popupshow">
                ...
                <div class="busin-but">
                    <button class="rise" v-if="!orderlist.direction" @click="dialog">市价买多</button>
                    <button class="fall" v-else @click="dialog">市价卖空</button>
                </div>
            </div>
        </transition>
...
export default {
    ...
    watch: {
        ...
        'orderlist.hands'(v){//手数
            ...
            //  console.log(v>this.list[this.swiperindex].sumHandsLimit);
    ...
    methods: {
        ...
        dialog(){//风险提示
            ...
            this.$dialog.confirm({
                title: '风险提示',
                message: `1、由于合约交易的点差机制，任何交易品种都会有三种价格，分别为“市价"“买多价”“卖空价”，买空价与卖空价分别是距市价加或减市价的万分之十二的距离，当用户对交易品种进行买多时以”买多价“为成本价进行成交，此时”卖空价“即为您的平仓结算价，包含您设置止损止盈时也请以”卖空价“为准；当用户对交易品种进行卖空时以”卖空价“为成本价成交，此时”买多价“即为您的平仓结算价，包含您设置止损止盈时也请以”买多价“为准
                            2、当您的账户风险率达到或低于70%时，为避免您的账户因行情原因造成更大的损失，系统将会强制平仓您的所有持有订单，账户风险率计算方式为：账户净值/占用USDT*100%
                            3、数字资产合约交易具有杠杆存在，交易损失风险可能性很大，有可能导致您损失账户内部分或全部USDT，因此您应该根据您的财务状况仔细考虑数字资产合约杠杆交易是否适合您`
                }).then(() => {
                // on confirm
                this.order()
                }).catch(() => {
        ...
        klineUpdate(){//更新k线数据
            ...
            if (this.intervalact===1) {//如果为分时线
                // this.line.fsupdate(time,this.market.price)
            }else{
                var interval=this.intervalact
                interval=='1h'?interval=60:interval
                interval=='1d'?interval=60*24:interval
                console.log(new Date(this.data[this.data.length-1].timestamp+interval*60*1000),'k');
                this.data[this.data.length-1].timestamp+interval*60*1000
                time
                // this.line.kupdate(interval)
            }
        ...
        getlatestdeal(){//获取最新订单
            ...
            this.$axios.post(`api/trade/contract/trade_query?start=0&rows=20&status=0`).then(res=>{
                // console.warn(res);
                if (!res.data.errorCode) {
