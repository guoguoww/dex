        ...
        <van-list v-model="loading" :finished="finished" :immediate-check="immediateCheck"  @load="onLoad">
          <van-cell v-for="(item,index) in transferLists" :key="index">
            <ul class="transferInfos">
              <li>
                <span>{{item.assetsType[0]=='0'?'C2C':item.assetsType[0]=='1'?'币币':item.assetsType[0]=='2'?'合约':'无效'}} => {{item.assetsType.split(',')[1]=='0'?'C2C':item.assetsType.split(',')[1]=='1'?'币币':item.assetsType.split(',')[1]=='2'?'合约':'无效'}}</span>
  ...
  export default {
    ...
    data(){
      return {
        ...
        modeInfos: [{name:'C2C',id:'0'},{name:'合约',id:'2'}],
        noData: true,
    ...
    mounted(){
      ...
      this.initFromType = this.$route.query.curType == '0'?'C2C':
        this.$route.query.curType == '1'?'币币':
          this.$route.query.curType == '2'?'合约 ':'全部';
      ...
      getTransfer(){//资金划转的接口
        let targetAddress = this.intToType.charCodeAt(0) == 'C2C'.charCodeAt(0)? 0 :
          this.intToType.charCodeAt(0) == '币币'.charCodeAt(0)? 1 :
            this.intToType.charCodeAt(0) == '合约'.charCodeAt(0)? 2 : -1;
