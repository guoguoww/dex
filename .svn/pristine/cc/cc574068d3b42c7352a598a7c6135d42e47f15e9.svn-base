...
class line {
  ...
  fsupdate(time, item) {
    ...
    var times = option.xAxis[0].data
    var leng = data.length - 1
    //console.log(option.series[0].data, option.xAxis[0].data, 'update');
    data.pop()
    data.push(item)
    times.pop()
    times.push(time)
   ...
   * k线更新数据
   * @param {String} time 时间
   * @param {Array} item 数据 [o,c,h,l]
   */
  kupdate(time,item){
    var option = this.chart.getOption()
    var data = option.series[0].data
    var times = option.xAxis[0].data
    console.log(data, +times[times.length - 1], 'klineupdate', time);
